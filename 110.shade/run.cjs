...
var open = async ( prt ) =>{
  ...
  require("../110.shade/110.shade/000.quest.shade");
  ...
  //bit = await TIME.hunt(ActTme.INIT_TIME , { dat:MQTT , src:local});
  bit = await SHADE.hunt( SHADE.ActShd.INIT_SHADE , { val: 1, dat: MQTT, src:  [localBit, remoteBit]  });
